...
#		lbroken[8]=lbroken[8]+":RAC:SER1:SER2:SPOIS:SFET:BINOM:PIR:MC:nRL:PHA:SQ:RDX" #Add new fields names
		lbroken[8]=lbroken[8]+":RAC:SER1:SER2:SPOIS:SFET:BINOM:PIR:MC:nRL:PHA"
		col=9	
		for bam in bams:
			print(chr,p,bam,col)
			...
			if "0" not in lbroken[col].split(":")[0] and "1" not in lbroken[col].split(":")[0]:
				...
				#Alleles depth
				ADfn=lbroken[8].split(":").index("AD")
				AD=list(map(int,lbroken[col].split(":")[ADfn].split(",")))
...
#				lbroken[col]=":".join([item for sublist in [lbroken[col].split(":")[:5],[",".join(str(x) for x in SAC)],lbroken[col].split(":")[6:]]for item in sublist])				
				#Vardict
				SAC=[]
				for i in lbroken[9].split(":")[lbroken[8].split(":").index("RD")].split(","): SAC.append(int(i))
				for i in lbroken[9].split(":")[lbroken[8].split(":").index("ALD")].split(","): SAC.append(int(i))
				RACsums=[]
...
#				lbroken[col]=":".join(str(x) for x in [lbroken[col].replace("\n",""),",".join(str(x) for x in RAC),SER1,SER2,SPOIS,SFET,BINOM,",".join(str(x) for x in PIR),MC,annotationn,annotationh,XHMM])
				lbroken[col]=":".join(str(x) for x in [lbroken[col].replace("\n",""),",".join(str(x) for x in RAC),SER1,SER2,SPOIS,SFET,BINOM,",".join(str(x) for x in PIR),MC,annotationn,annotationh])
			else:
				if SAC=="." or sum(SAC)==0:
#					lbroken[col]=":".join(str(x) for x in [lbroken[col].replace("\n",""),",".join(str(x) for x in RAC),float(("%0.4f"%SER1)),float(("%0.4f"%SER2)),SPOIS,SFET,float(("%0.4f"%BINOM)),",".join(str(x) for x in PIR),MC,annotationn,annotationh,XHMM])
					lbroken[col]=":".join(str(x) for x in [lbroken[col].replace("\n",""),",".join(str(x) for x in RAC),float(("%0.4f"%SER1)),float(("%0.4f"%SER2)),SPOIS,SFET,float(("%0.4f"%BINOM)),",".join(str(x) for x in PIR),MC,annotationn,annotationh])
				else:
#					lbroken[col]=":".join(str(x) for x in [lbroken[col].replace("\n",""),",".join(str(x) for x in RAC),float(("%0.4f"%SER1)),float(("%0.4f"%SER2)),float(("%0.4f"%SPOIS)),float(("%0.4f"%SFET)),float(("%0.4f"%BINOM)),",".join(str(x) for x in PIR),MC,annotationn,annotationh,XHMM])
					lbroken[col]=":".join(str(x) for x in [lbroken[col].replace("\n",""),",".join(str(x) for x in RAC),float(("%0.4f"%SER1)),float(("%0.4f"%SER2)),float(("%0.4f"%SPOIS)),float(("%0.4f"%SFET)),float(("%0.4f"%BINOM)),",".join(str(x) for x in PIR),MC,annotationn,annotationh])
			print(col,"annotated")
			col+=1
